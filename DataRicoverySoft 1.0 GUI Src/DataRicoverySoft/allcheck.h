...
void fName()
{
           ...
           // strcpy(filename1, "Recovered Files\\R");
			  sprintf(filename1,"%s\\R",Tfil);
            strcat(filename1, RandomName);
...
{        
	        start_file=loop; 
             ...
             sprintf(ResultV,"File Found..., Save As :%s",filename1);
			 SetDlgItemText(MainW,IDC_STATIC3,ResultV);
             FileNo++;
...
{ 
                       ...
                       sprintf(ResultV,"Calcute Size : %ld KB",(file_size)/1024);
					   SetDlgItemText(MainW,IDC_STATIC4,ResultV);
